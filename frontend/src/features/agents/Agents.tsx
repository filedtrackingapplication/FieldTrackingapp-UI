  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`${selectedAgent ? 'btn-primary flex items-center gap-2' : 'btn-primary flex items-center gap-2 opacity-50 cursor-not-allowed'}`}
          >
            <UserPlus className="w-4 h-4" /> Edit Agent
          </button>
          <ImportTemplateButtons
            accept=".csv"
            onFile={(f)=>setCsvFile(f)}
            onUpload={async ()=>{
              if (!csvFile) { toast.error('Select CSV first'); return }
            ...
            templateFilename={'agents_template.csv'}
            templateContent={'full_name,email,phone,employee_id,role\nAgent One,agent1@example.com,9991112222,AG001,field_agent\n'}
          />
        </div>
      </div>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <DataTable
            ...
            onRowClick={(row)=> setSelectedAgent(row as any)}
      ...
      <AddAgentModal
        ...
        onSuccess={() => { load(); setSelectedAgent(null) }}
        mode="edit"
